...
namespace fhInventoryEditor
{
    public partial class Form1 : Form
    {
        ...
        public enum DocumentType { none, quote, delivery }
        public enum DocumentLanguage { none, english, french }
        public readonly struct ClientForm
        {
            public DocumentType Type { get; }
            public DocumentLanguage Language { get; }
            public Table Products { get; }
            public Table Contacts { get; }
            public FileInfo Path { get; }
            public ClientForm(string fullpath, bool openFile = false)
            {
                var clientform = new ClientForm(new FileInfo(fullpath), openFile);
                Type = clientform.Type;
                Language = clientform.Language;
                Products = clientform.Products;
                Contacts = clientform.Contacts;
                Path = clientform.Path;
            }
            public ClientForm(FileInfo jobinfo, bool openFile = false)
            {
                Type = DocumentType.none;
                Language = DocumentLanguage.none;
                
                Products = new Table() { Name = "products_" + jobinfo.Directory.Name };
                var item = Products.Columns.Add("item", typeof(string));
                var desc = Products.Columns.Add("desc", typeof(string));
                var qty = Products.Columns.Add("qty", typeof(double));
                var path = Products.Columns.Add("path", typeof(string));
                var docDate = Products.Columns.Add("date", typeof(DateTime));
                Products.PrimaryKeys = new Table.Column[] { item };

                Contacts = new Table() { Name = "contacts_" + jobinfo.Directory.Name };
                var nameColumn = Contacts.Columns.Add("name", typeof(string));
                var typeColumn = Contacts.Columns.Add("type", typeof(string));
                var keyColumn = Contacts.Columns.Add("key", typeof(string));
                var valueColumn = Contacts.Columns.Add("value", typeof(string));
                Contacts.PrimaryKeys = new Table.Column[] { keyColumn };
                ...
                if (jobinfo != null)
                {
                    ...
                    {
                        var typeAndLanguage = Get_documentTypeLanguage(document);
                        Type = typeAndLanguage.type;
                        Language = typeAndLanguage.language;
                            ...
                            foreach (var line in words_thisPage.Where(w => w.Value.Any()))
                            {
                                ...
                                else if (pgRegion == PageRegion.table_data)
                                {
                                    ...
                                    if (colIndex == 0)
                                    {
                                        ...
                                        Products.Rows.Add(new object[] { cell1_item, cell2_desc, qtyCell, jobinfo.Name, jobinfo.CreationTime });
                        ...
                        Products.Name = $"{typeAndLanguage}_{jobinfo.Directory.Name}";
                        ...
                        var fileTuple = Tuple.Create(contacts, Products);
                        ...
                        foreach (var kvp in contacts)
                            Contacts.Rows.Add(new string[] { jobinfo.Name, typeAndLanguage.ToString(), kvp.Key, kvp.Value });
                    }
                }
            }
            public override string ToString() => $"{Language} {Type} {Products.Rows.Count} Products {Contacts.Rows.Count} Contacts";
        }
        public struct TypeAndLanguage
        {
        ...
        private static Dictionary<int, Dictionary<byte, string>> Get_pdfText(PdfDocument document)
        {
            var pages = new Dictionary<int, Dictionary<byte, string>>();
            foreach (var page in document.GetPages())
                pages[page.Number] = Get_pageText(page);
            return pages;
        }
            ...
            if (directory != null)
            {
                ...
                if (forms.Item3.Any())
                {
                    ...
                    if (deliveryFiles.Any())
                    {
                        ...
                        var deliveryForm = new ClientForm(deliveryFiles.Last());
                        ...
                        var businessName = ProperCase(deliveryForm.Contacts.AsEnumerable.Where(r => r["key"].ToString() == "Customer").Select(r => r["value"].ToString()).First());
                        var contact_name = ProperCase(deliveryForm.Contacts.AsEnumerable.Where(r => r["key"].ToString() == "Contact").Select(r => r["value"].ToString()).First());
                        var contact_phone = deliveryForm.Contacts.AsEnumerable.Where(r => r["key"].ToString() == "Phone#").Select(r => r["value"].ToString()).First();

                        // get address data from the quote form
                        var quotesToMatch = new List<ClientForm>();
                        var quoteOrders = new List<ClientForm>(quoteForms.Where(qf => qf.Contacts.AsEnumerable.Where(r => r["key"].ToString() == "Order#" & r["value"].ToString().Contains(orderNbr)).Any()));
                        if (quoteOrders.Any())
                            quotesToMatch.AddRange(quoteOrders); // matching PO#
                        else
                            quotesToMatch.AddRange(quoteForms); // no match- see below /// explanation
                        ...
                        foreach (var quoteForm in quotesToMatch)
                        {
                            var codesQuoted = new List<string>(quoteForm.Products.Columns["item"].Values.Select(i => i.Value.ToString()));
                            var codesDelivered = new List<string>(deliveryForm.Products.Columns["item"].Values.Select(i => i.Value.ToString()));
                            ...
                            hitRates.Add(Tuple.Create(hitRate, quoteForm));
                        }
                        ...
                        var contacts = new Table();
                        hitRates.Sort((q1, q2) => q2.Item1.CompareTo(q1.Item1));
                        if (hitRates.Any())
                        {
                            var bestQuote = hitRates.First();
                            contacts = bestQuote.Item2.Contacts;
                            address_street = ProperCase(string.Join(" ", contacts.AsEnumerable.Where(r => r["key"].ToString().StartsWith("Address")).Select(r => r["value"].ToString())));
                            address_city = ProperCase(contacts.AsEnumerable.Where(r => r["key"].ToString() == "City").Select(r => r["value"].ToString()).First());
                            address_province = provinces.Where(p => p.Item1 == ProperCase(contacts.AsEnumerable.Where(r => r["key"].ToString() == "Province").Select(r => r["value"].ToString()).First())).First().Item3;
                            address_postal = ProperCase(contacts.AsEnumerable.Where(r => r["key"].ToString() == "PostalCode").Select(r => r["value"].ToString()).First());
                            contact_email = ProperCase(contacts.AsEnumerable.Where(r => r["key"].ToString() == "email-client").Select(r => r["value"].ToString()).First());
                        }
                        else
                        {
                            // going to have to get address and email from somewhere else (internet??)
                            Debugger.Break();
                        }

                        // ----------- these elements come from an INVOICE
                        var beginDate = new DateTime();
                        var endDate = new DateTime();
                        var billDirectory = directory.CreateSubdirectory("billing");
                        var relatedInvoices = new List<Invoice>(invoices.Where(i => i.Valid & i.Date > deliveryForm.Path.CreationTime)); // reference the PO# on your invoices??
                        if (relatedInvoices.Any())
                        {
                            relatedInvoices.Sort((i1, i2) => i1.Date.CompareTo(i2.Date));
                            var relatedInvoice = relatedInvoices.First();
                            beginDate = relatedInvoice.Date_begin;
            ...
            foreach (var jobFolder in validFolders)
            {
                ...
                foreach (var dlvryFile in dlvryFiles.Take(100))
                {
                    try
                    {
                        ...
                        tables.Add(dlvryFile.Name, xx.Products.HTML);
                    }
                    catch { }
                }
                foreach (var quoteFile in quoteFiles)
                {
                    try
                    {
                        ...
                        tables.Add(quoteFile.Name, xx.Products.HTML);
                    }
        ...
        internal static Dictionary<DocumentType, List<FileInfo>> Get_DocumentTypes(DirectoryInfo jobfolder)
        {
            ...
            foreach (var pdf in jobFiles)
                try
                {
                    using (PdfDocument document = PdfDocument.Open(pdf.FullName))
                    {
                        var typeAndLanguage = Get_documentTypeLanguage(document);
                        if (!doctypes.ContainsKey(typeAndLanguage.type)) doctypes[typeAndLanguage.type] = new List<FileInfo>();
                        doctypes[typeAndLanguage.type].Add(pdf);
                    }
                }
                catch { }
            return doctypes;
        ...
        private static TypeAndLanguage Get_documentTypeLanguage(PdfDocument pdf)
        {
            ...
            var documentPages = Get_pdfText(pdf);
            if (!documentPages.Any()) return new TypeAndLanguage() { language = DocumentLanguage.none, type = DocumentType.none };
            var documentText = string.Join(new string('-', 50), documentPages.Values.Select(p => string.Join(Environment.NewLine, p.Values)));
            ...
            if (quoteMatch.Success)
            {
                ...
                return new TypeAndLanguage() { language = quoteLanguage, type = DocumentType.quote };
            }
            ...
            if (dlvryMatch.Success)
            {
                ...
                return new TypeAndLanguage() { language = deliveryLanguage, type = DocumentType.delivery };
            }
            ...
            return new TypeAndLanguage() { language = noneLanguage, type = DocumentType.none };
        }
        ...
        private static Tuple<Table, Table, List<FileInfo>> Parse_forms(DirectoryInfo jobFolder, bool openFiles = false)
        {
            ...
            foreach (var file in jobFolder.EnumerateFiles("*.pdf").Where(f=>Regex.IsMatch(f.Name, "(delivery|quote)_[^.]{1,}\\.pdf")))
            {
                ...
                folderTable.Merge(parsedForm.Products);
                files.Add(file);
                contactsTable.Merge(parsedForm.Contacts);
            }
            return Tuple.Create(folderTable, contactsTable, files);
        }
        private static ClientForm Parse_form(FileInfo jobinfo, bool openFile = false) => new ClientForm(jobinfo, openFile);
